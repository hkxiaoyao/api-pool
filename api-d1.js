...
async function generateJWT(env) {
  ...
  if (STORAGE_TYPE === "d1" && env.DB) {
    try {
      // 从D1获取密钥
      const result = await env.DB.prepare("SELECT value FROM settings WHERE key = ?").bind(KV_KEYS.SESSION_SECRET).first();
      secretKey = result ? result.value : null;
      ...
      if (!secretKey) {
        ...
        await env.DB.prepare("INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = CURRENT_TIMESTAMP")
          .bind(KV_KEYS.SESSION_SECRET, secretKey)
          .run();
      }
    } catch (dbError) {
      Logger.error("从D1获取会话密钥失败:", dbError);
      // 如果D1失败，尝试从KV获取作为备份
      secretKey = await env.API_TOKENS.get(KV_KEYS.SESSION_SECRET);
    }
  } else {
    // 从KV获取密钥
    secretKey = await env.API_TOKENS.get(KV_KEYS.SESSION_SECRET);
...
// 从KV迁移数据到D1
async function migrateFromKVToD1(env) {
  if (!env || !env.DB || !env.API_TOKENS) return false;

  try {
    Logger.info("检查是否需要从KV迁移数据到D1...");

    // 检查D1中是否已有数据
    const tokensExist = await env.DB.prepare("SELECT COUNT(*) as count FROM tokens").first();
    const statsExist = await env.DB.prepare("SELECT COUNT(*) as count FROM stats").first();
    const settingsExist = await env.DB.prepare("SELECT COUNT(*) as count FROM settings WHERE key = ?").bind(KV_KEYS.SESSION_SECRET).first();

    // 如果D1中已有数据，则不需要迁移
    if (tokensExist && tokensExist.count > 0 && statsExist && statsExist.count > 0 && settingsExist && settingsExist.count > 0) {
      Logger.info("D1数据库中已有数据，无需迁移");
      return true;
    }

    Logger.info("开始从KV迁移数据到D1...");

    // 迁移tokens数据
    if (!tokensExist || tokensExist.count === 0) {
      const tokensData = await env.API_TOKENS.get(KV_KEYS.TOKENS, { type: "json" });
      if (tokensData) {
        await env.DB.prepare("INSERT INTO tokens (data) VALUES (?)").bind(JSON.stringify(tokensData)).run();
        Logger.info(`已迁移${tokensData.length}个令牌从KV到D1`);
      }
    }

    // 迁移stats数据
    if (!statsExist || statsExist.count === 0) {
      const statsData = await env.API_TOKENS.get(KV_KEYS.STATS, { type: "json" });
      if (statsData) {
        await env.DB.prepare("INSERT INTO stats (request_timestamps, token_counts, request_timestamps_day, token_counts_day, last_processed_date) VALUES (?, ?, ?, ?, ?)")
          .bind(
            JSON.stringify(statsData.requestTimestamps || []),
            JSON.stringify(statsData.tokenCounts || []),
            JSON.stringify(statsData.requestTimestampsDay || []),
            JSON.stringify(statsData.tokenCountsDay || []),
            statsData.lastProcessedDate || null
          )
          .run();
        Logger.info("已迁移统计数据从KV到D1");
      }
    }
    ...
    Logger.error("从KV迁移数据到D1失败:", error);
    return false;
  }
}
      ...
      try {
        ...
        await env.DB.prepare(
          ...
          CREATE TABLE IF NOT EXISTS stats (
            ...
            request_timestamps_day TEXT NOT NULL,
            token_counts_day TEXT NOT NULL,
        ...
        Logger.info("D1数据库表初始化完成");

        // 尝试从KV迁移数据到D1
        await migrateFromKVToD1(env);
      } catch (error) {
        Logger.error("初始化D1数据库表失败:", error);
      }
    }
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      try {
        ...
        if (STORAGE_TYPE === "d1" && env.DB) {
          try {
            // 清理tokens表，只保留最新的5条记录
            await env.DB.prepare("DELETE FROM tokens WHERE id NOT IN (SELECT id FROM tokens ORDER BY id DESC LIMIT 5)").run();

            // 清理stats表，只保留最新的10条记录
            await env.DB.prepare("DELETE FROM stats WHERE id NOT IN (SELECT id FROM stats ORDER BY id DESC LIMIT 10)").run();

            Logger.info("D1数据库维护任务完成：已清理旧记录");
          } catch (dbError) {
            Logger.error("D1数据库维护任务失败:", dbError);
          }
        }
